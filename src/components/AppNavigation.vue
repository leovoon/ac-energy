...
const allNavigationItems: NavigationItem[] = [
  ...
  },
  {
    ...
    label: 'Analytics',
    icon: 'BarChartIcon',
    roles: [UserRole.Parent, UserRole.Child], // Only visible to Parent and Child roles
    order: {
      [UserRole.Admin]: 99, // Not visible to admin
      [UserRole.Parent]: 1,
      [UserRole.Child]: 1
    }
  },
...
<template>
  <SidebarProvider>
    <Sidebar collapsible="offcanvas" class="h-screen border-r">
      <SidebarHeader>
        <div class="p-4">
          <h1 class="text-xl font-bold">AC Energy</h1>
            ...
            <SidebarMenu>
              <SidebarMenuItem v-for="item in navigationItems" :key="item.name">
                <SidebarMenuButton
                  ...
                  @click="navigateAndCloseSidebar(item.name)"
                >
    ...
    <!-- Mobile sidebar trigger that will appear in header on small screens -->
    <SidebarTrigger class="lg:hidden md:block" />
  </SidebarProvider>
</template>